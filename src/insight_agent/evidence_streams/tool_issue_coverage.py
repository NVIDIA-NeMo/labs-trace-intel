...
def _field_presence(records: Sequence[Mapping[str, Any]]) -> dict[str, int]:
    ...
    for record in records:
        for key in (
            "tool_catalog", "steps", "logical_case_id", "source_pointer",
            "observed_verdict", "cost", "metrics",
            "task_text", "orphan_results",
        ):
            if record.get(key):
                counts[key] += 1
...
def corpus_coverage(
    ...
    *,
    ...
    if presence["steps"] == 0:
        notes.append(
            "No record has steps: IA2 trajectory tokens fall back to one token per call."
        )
